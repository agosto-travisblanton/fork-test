...
class TenantsHandler(RequestHandler):
    ...
    def post(self):
        if self.request.body is not str('') and self.request.body is not None:
            ...
            notification_emails = comma_delimited_string_to_list(request_json.get('notification_emails'))
    ...
    def put(self, tenant_key):
        ...
        tenant.notification_emails = comma_delimited_string_to_list(request_json.get('notification_emails'))
        domain_key_input = request_json.get('domain_key')
        try:
            domain_key = ndb.Key(urlsafe=domain_key_input)
        except Exception, e:
