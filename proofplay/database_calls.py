...
def delete_raw_event_entries_older_than_thirty_days(retries=0):
    try:
        ...
        time.sleep(1)
        if retries < 3:
            return delete_raw_event_entries_older_than_thirty_days(retries=retries + 1)
...
def retrieve_all_devices_of_tenant(tenant, retries=0):
    try:
        ...
        time.sleep(1)
        if retries < 3:
            return retrieve_all_devices_of_tenant(tenant=tenant, retries=retries + 1)
...
def retrieve_all_locations_of_tenant(tenant, retries=0):
    try:
        ...
        time.sleep(1)
        if retries < 3:
            return retrieve_all_locations_of_tenant(tenant=tenant, retries=retries + 1)
...
def retrieve_customer_location_code_from_location_id(location_id, retries=0):
    try:
        ...
        time.sleep(1)
        if retries < 3:
            return retrieve_customer_location_code_from_location_id(location_id=location_id, retries=retries + 1)
...
def retrieve_all_resources_of_tenant(tenant, retries=0):
    try:
        ...
        time.sleep(1)
        if retries < 3:
            return retrieve_all_resources_of_tenant(tenant=tenant, retries=retries + 1)
...
def retrieve_resource_name_from_resource_identifier(modified_resource_identifier, retries=0):
    try:
        ...
        time.sleep(1)
        if retries < 3:
            return retrieve_resource_name_from_resource_identifier(
                modified_resource_identifier=modified_resource_identifier, retries=retries + 1)
...
def retrieve_all_resources(retries=0):
    try:
        ...
        time.sleep(1)
        if retries < 3:
            return retrieve_all_resources(retries=retries + 1)
...
def insert_raw_program_play_event_data(each_log, retries=0):
    try:
        ...
        time.sleep(1)
        if retries < 3:
            return insert_raw_program_play_event_data(each_log=each_log, retries=retries + 1)
...
def mark_raw_event_complete(raw_event_id, retries=0):
    try:
        ...
        time.sleep(1)
        if retries < 3:
            return mark_raw_event_complete(raw_event_id=raw_event_id, retries=retries + 1)
...
def insert_new_program_record(location_id, device_id, resource_id, started_at, ended_at, retries=0):
    try:
        ...
        time.sleep(1)
        if retries < 3:
            return insert_new_program_record(location_id, device_id, resource_id, started_at, ended_at,
                                             retries=retries + 1)
...
def insert_new_resource_or_get_existing(resource_name, resource_identifier, tenant_code, retries=0):
    try:
        ...
        time.sleep(1)
        if retries < 3:
            return insert_new_resource_or_get_existing(resource_name, resource_identifier, tenant_code,
                                                       retries=retries + 1)
...
def insert_new_tenant_code_or_get_existing(tenant_code, retries=0):
    try:
        ...
        time.sleep(1)
        if retries < 3:
            return insert_new_tenant_code_or_get_existing(tenant_code=tenant_code, retries=retries + 1)
...
def insert_new_location_or_get_existing(customer_location_code, tenant_id, retries=0):
    try:
        ...
        time.sleep(1)
        if retries < 3:
            return insert_new_location_or_get_existing(customer_location_code, tenant_id, retries=retries + 1)
...
def insert_new_device_or_get_existing(location_id, serial_number, device_key, customer_display_code, tenant_code,
    ...
    try:
        ...
        time.sleep(1)
        if retries < 3:
            return insert_new_device_or_get_existing(location_id, serial_number, device_key, customer_display_code,
                                                     tenant_code, retries=retries + 1)
...
def get_raw_program_record_data_by_resource(start_date, end_date, resource_identifier, tenant_code, retries=0):
    try:
        ...
        time.sleep(1)
        if retries < 3:
            return get_raw_program_record_data_by_resource(start_date, end_date, resource_identifier, tenant_code,
                                                           retries=retries + 1)
...
def get_raw_program_record_data_by_device(start_date, end_date, customer_display_code, tenant_code, retries=0):
    try:
        ...
        time.sleep(1)
        if retries < 3:
            return get_raw_program_record_data_by_device(start_date, end_date, customer_display_code, tenant_code,
                                                         retries=retries + 1)
...
def get_raw_program_record_data_by_location(start_date, end_date, customer_location_code, tenant_code, retries=0):
    try:
        ...
        time.sleep(1)
        if retries < 3:
            return get_raw_program_record_data_by_location(start_date, end_date, customer_location_code, tenant_code,
                                                           retries=retries + 1)
