...
class BaseTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if 'USE_RANDOM_CONSISTENCY_POLICY' in os.environ.keys():
            probability_str = os.environ['RANDOM_CONSISTENCY_PROBABILITY']
            if probability_str is not None:
                self.policy = self.policy = datastore_stub_util.PseudoRandomHRConsistencyPolicy(
                    probability=float(probability_str))
            else:
                self.policy = datastore_stub_util.PseudoRandomHRConsistencyPolicy(probability=0.0)
            import logging

            logging.warn(
                "setting PseudoRandomHRD consistency policy percentage to {}%".format(100 * self.policy._probability))
            self.testbed.init_datastore_v3_stub(consistency_policy=self.policy, require_indexes=True,
                root_path=get_project_root())
        else:
            self.testbed.init_datastore_v3_stub(require_indexes=True, root_path=get_project_root())
        self.testbed.init_memcache_stub()
        self.testbed.init_taskqueue_stub(root_path=get_project_root(), _all_queues_valid=True)
    ...
    def tearDown(self):
        # deactivate testbed; also has effect of clearing any environment variables set during the test.
    ...
    def get_sent_messages(self, to=None, sender=None, subject=None, body=None, html=None):
        ...
        """
        return self.testbed.get_stub('mail').get_sent_messages(to=to, sender=sender, subject=subject, body=body,
            html=html)
    ...
    def assertRunAndClearTasksInQueue(self, n=None, url=None, name=None, queue_names=None):
        """
        Will assert the number of tasks you specify in the named queue(s) or ALL queues if queue_names are None. It
        will then run those tasks. If you specified a queue_names list or a scalar queue name, it will then delete those
        tasks only from those queue(s) specified or from ALL queues in one sweep if unspecified.

        Example::
            deferred.defer(some_task, _queue='my-queue')
            self.assertRunAndClearTasksInQueue(1, queue_names='my-queue')

        Example::
            deferred.defer(some_task, _queue='my-queue')
            deferred.defer(some_other_task, _queue='my-other-queue')
            self.assertRunAndClearTasksInQueue(2, queue_names=['my-queue', 'my-other-queue'])

        :param n: number of tasks
        :param url:
        :param name: name of task
        :param queue_names: list or scalar name of queue(s)
        :return: None
        """
        tasks = self.get_tasks()
        self.assertTasksInQueue(n, url, name, queue_names)
        self.run_deferred_tasks(name, queue_names)

        q = self.testbed.get_stub('taskqueue')
        if queue_names is None:
            queue_names = [queue['name'] for queue in q.GetQueues()]
        elif not hasattr(queue_names, '__iter__'):
            queue_names = [queue_names]

        for task in tasks:
            for queue_name in queue_names:
                q.DeleteTask(queue_name, task.name)
    ...
    def clear_queues(self, queue_names=None):
        q = self.testbed.get_stub('taskqueue')
        if None is queue_names:
            queue_names = [queue['name'] for queue in q.GetQueues()]
        elif not hasattr(queue_names, '__iter__'):
            queue_names = [queue_names]
        for queue_name in queue_names:
            q.FlushQueue(queue_name=queue_name)
    ...
    def run_deferred_tasks(self, name=None, queue_names=None):
        """
        Executes a list of `Deferred Task`_ objects with the specified criteria.

        :param name: name criteria tasks must match. If ``name`` is ``None``, all tasks will be matched.
        :param queue_names: queue name criteria tasks must match. If ``queue_name`` is ``None`` tasks in all queues will be matched.

        """
        tasks = self.get_tasks(name=name, queue_names=queue_names)
        for task in tasks:
